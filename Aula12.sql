...
DECLARE
    ...
    emp_salario REAL;
    emp_atual_nome TEXT;
    emp_atual_salario REAL;
BEGIN
    emp_nome := '';
    emp_salario := 0;
    
    FOR emp_atual_nome, emp_atual_salario IN
        SELECT nome, salario
        FROM tbl_empregados
    LOOP
        IF emp_atual_salario > emp_salario THEN
            emp_nome := emp_atual_nome;
            emp_salario := emp_atual_salario;
        END IF;
    END LOOP;
...
BEGIN
    ...
    SELECT COUNT(*) INTO total_empregados
    FROM tbl_empregados;
...
RETURNS TABLE(salario REAL, salario_com_10_porcento REAL, salario_com_15_porcento REAL) AS $$
BEGIN
    RETURN QUERY
    SELECT 
        e.salario::real,  
        (e.salario * 1.10)::real AS salario_com_10_porcento,  
        (e.salario * 0.85)::real AS salario_com_15_porcento  
    FROM tbl_empregados e  
    WHERE e.cod_empregado = $1;  
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    INTO qtd_projetos
    FROM tbl_projetos
    WHERE tbl_projetos.cod_departamento = $1;  -- Usando o parâmetro corretamente
...
SELECT fc_proj_departamento(10);
