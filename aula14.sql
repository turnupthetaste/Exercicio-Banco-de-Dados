...
CREATE OR REPLACE FUNCTION atualizar_qtde_projetos_deletado() 
RETURNS TRIGGER AS $$
BEGIN
    -- Atualiza a quantidade de projetos do departamento após a exclusão de um projeto
...
CREATE TRIGGER trg_atualizar_qtde_projetos_deletado
AFTER DELETE ON tbl_projetos
FOR EACH ROW
EXECUTE FUNCTION atualizar_qtde_projetos_deletado();


--2 crie uma trigger que atualiza a quantidade de projetos da tbl_departamentos caso um novo projeto for adicionado

CREATE OR REPLACE FUNCTION atualizar_qtde_projetos_adicionado() 
RETURNS TRIGGER AS $$
BEGIN
    -- Atualiza a quantidade de projetos do departamento após a inserção de um novo projeto
    UPDATE tbl_departamentos
    SET qtde_projetos = qtde_projetos + 1
...
-- Criação da trigger que chama a função acima
CREATE TRIGGER trg_atualizar_qtde_projetos_adicionado
AFTER INSERT ON tbl_projetos
FOR EACH ROW
EXECUTE FUNCTION atualizar_qtde_projetos_adicionado();
...
RETURNS TRIGGER AS $$
BEGIN
    -- Insere um novo registro na tabela de histórico sempre que o salário for atualizado
...
RETURNS TRIGGER AS $$
BEGIN
    -- Atualiza a coluna data_atualizacao toda vez que os dados do empregado forem atualizados
