...
AS $$
BEGIN
    -- Atualiza o nome do departamento com o código fornecido
...
AS $$
BEGIN
    -- Cria uma cópia da tabela tbl_cidades com o nome tbl_cidades_copy
    EXECUTE 'CREATE TABLE tbl_cidades_copy AS TABLE tbl_cidades';
END;
$$;
...
AS $$
BEGIN
    -- Insere um novo projeto na tabela tbl_projetos
...
AS $$
BEGIN
    -- Deleta o projeto da tabela tbl_projetos com o código fornecido
...
AS $$
BEGIN
    -- Verifica se a tabela tbl_projetos_arquivados existe. Se não, cria a tabela.
    ...
    -- Insere o projeto na tabela tbl_projetos_arquivados
